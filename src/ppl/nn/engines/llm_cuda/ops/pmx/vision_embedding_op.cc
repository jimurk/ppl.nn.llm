...
#include "ppl/nn/engines/llm_cuda/kernels/pmx/vision_embedding_kernel.h"
...
RetCode VisionEmbeddingOp::CommonInit() {
    ...
    infer_dims_func_ = [this](InputOutputInfo* info) -> RetCode {
        return nn::pmx::ReshapeVisionEmbedding(info, param_.get());
    };
    return RC_SUCCESS;
}
...
ppl::common::RetCode VisionEmbeddingOp::SerializeData(const ppl::nn::pmx::SerializationContext&, utils::DataStream* ds) const {
    ...
    auto fb_param = pmx::CreateVisionEmbeddingParam(builder, 
        param_.get()->hidden_dim,
        param_.get()->image_size,
        param_.get()->patch_size);
    auto fb_op_param = pmx::CreateOpParam(builder, pmx::OpParamType_VisionEmbeddingParam, fb_param.Union());
